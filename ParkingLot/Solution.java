package ParkingLot;
   ...
   class ParkingFloor
   {
       ...
       // it improves complexity of park method from o(row*col) to o(1)
       private HashMap<Integer, ConcurrentLinkedDeque<ParkingSpot>> freeSpots;

       // vehicleType vs freeSpots size map
       private HashMap<Integer, AtomicInteger> freeSpotsSize;
       ...
       ParkingFloor(ParkingSpot[][] spots, Set<Integer> vehicleTypes)
       {
           ...
           for(ParkingSpot[] rows:spots)
               ...
               for(ParkingSpot spot: rows)if(spot!=null){
       ...
       public int removeVehicle(int row, int col)
       {
           ...
           if(spot.getVehicleNumber().isBlank() && spot.getTicketId().isBlank()) return 404;
           spot.setTicketId("");
   ...
   class SearchManager
   {
       ...
       {   // add prefixes to indiacate whether its spotId or vehicleId or ticketId
